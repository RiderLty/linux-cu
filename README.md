...
- **网络透传**：通过 `--recv` 双向 IPC 模式和 `send` 发送模式，可将管道数据通过网络传输，实现跨机器 HID 透传
...
4. **send**：读取指定 USB 设备数据，通过 UDS/UDP 发送到网络目标，并接收远端数据写回设备
5. **emulate**：
...
- 添加日志/过滤/转换中间层
- 支持多设备同时透传
- 远程透传（通过 `--recv` 双向 IPC 和 `send` 模式实现网络传输 PipeMsg）
...
`send` 子命令不创建 Gadget 设备，仅读取指定 USB 设备的数据并通过网络发送到目标，同时接收远端数据写回设备。
