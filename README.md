...
- **网络/IPC透传**：通过 `--recv` 双向 IPC 模式和 `send` 发送模式，可将管道数据通过/IPC传输，实现跨机器 HID 透传
...
# 双向 IPC 模式 (--recv)：接收注入 + 将主机输出回传给最近来源
sudo ./linux-cu load my_mouse.yaml --udp :9999 --recv --debug
sudo ./linux-cu load 046d_c08b.yaml --uds @hid --recv

# 发送模式：读取 USB 设备数据发送到网络/IPC目标
sudo ./linux-cu send 2:32 UDP:192.168.3.3:9981 --debug
sudo ./linux-cu send 046d:c08b UDS:@hid
sudo ./linux-cu send 2:32 UDP:127.0.0.1:9999
```
...
3. **load**：从 YAML 文件创建 Gadget 设备（无真实设备透传，仅支持 IPC 注入；`--recv` 启用双向模式）
4. **send**：读取指定 USB 设备数据，通过 UDS/UDP 发送到网络/IPC目标，并接收远端数据写回设备
5. **emulate**：
...
- 添加日志/过滤/转换中间层
- 支持多设备同时透传
- 远程透传（通过 `--recv` 双向 IPC 和 `send` 模式实现网络/IPC传输 PipeMsg）
...
通过 `--uds` 或 `--udp` 参数，可以开启外部事件注入功能，允许外部程序通过Unix域套接字或者UDP向 Gadget 设备注入 HID 数据。
...
`load` 子命令的 `--recv` 标志启用双向 IPC 模式。在此模式下：

1. **接收注入**：与普通 IPC 相同，接收 UDP/UDS 发来的 HID 数据注入 Gadget 设备
2. **回传主机输出**：将 Gadget 设备从主机收到的数据（如 LED 指令、Force Feedback）回传给最近一次 IPC 来源

回传目标以**最近一次接收数据的来源**为准。如果尚未收到任何 IPC 数据包，主机输出将被静默丢弃。

```
┌────────────────────────────────────────────────────────────┐
│  Machine A (load --recv)                                   │
│                                                            │
│  外部 ──UDP/UDS──▶ IPC监听 ──parse──▶ /dev/hidgN 写入      │
│  /dev/hidgN 读取 ──▶ buildPacket ──▶ LastTarget ──▶ 外部    │
│                         (回传到最近来源)                     │
└────────────────────────────────────────────────────────────┘
```
...
`send` 子命令不创建 Gadget 设备，仅读取指定 USB 设备的数据并通过网络/IPC发送到目标，同时接收远端数据写回设备。

设备格式与 `emulate` 相同（`bus:dev` 或 `VID:PID`），目标格式为 `PROTO:address`。

### 目标格式

| 格式 | 说明 |
|---|---|
| `UDS:@hid` | 抽象 Unix 域套接字 |
| `UDS:/tmp/hid.socket` | 文件 Unix 域套接字 |
| `UDP:192.168.3.3:9981` | UDP 目标地址 |
| `UDP:127.0.0.1:9999` | UDP 本地地址 |
...
│  Machine B (send)                                      │
│                                                        │
│  USB 设备读取 ──▶ buildPacket(iface) ──▶ UDP/UDS ──▶ 远端  │
│  远端 ──▶ UDP/UDS 读取 ──▶ parse ──▶ USB 设备写入          │
└────────────────────────────────────────────────────────┘
```

## 网络/IPC 透传示例

通过 `load --recv` 和 `send` 组合，可实现跨机器的 USB HID 透传：

```
┌──────────────────┐          UDP           ┌──────────────────┐
│   Machine A      │◄──────────────────────►│   Machine B      │
│   (Gadget 侧)    │    192.168.3.x:9981    │   (USB 设备侧)   │
│                  │                         │                  │
│  load mouse.yaml │                         │  send 2:5        │
│  --udp :9981     │                         │  UDP:192.168.3.1 │
│  --recv          │                         │  :9981           │
└──────────────────┘                         └──────────────────┘
```

Machine A 创建 Gadget 设备并监听 UDP 端口 9981，`--recv` 确保双向通信。
Machine B 读取真实 USB 鼠标数据，发送到 Machine A 的 9981 端口，同时接收回传的主机输出。
